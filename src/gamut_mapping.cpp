...
GamutMappingResult map_d65_xyz_with_oklch(const Xyz& input_xyz,
  ...
  if (options.intent == GamutMapIntent::OklchBoundaryProjection) {
    result.boundary_evidence_applicable = result.input_oklch.hue_defined;
    ...
    result.boundary_evidence_applicable = false;
...
GamutMappingResult map_d65_lab_to_gamut(const Lab& input,
  ...
  result.boundary_evidence_applicable =
      result.input_chroma > hue_defined_chroma;
  const double hue = result.boundary_evidence_applicable
                         ? lab_hue(normalized_input)
                         : 0.0;
  if (result.boundary_evidence_applicable) {
  ...
  // move an exact radial-boundary value by one ULP and make derived evidence
