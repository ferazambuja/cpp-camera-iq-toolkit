...
std::optional<ShadingGeometry> make_geometry(int width, int height,
...
bool same_exposure_value(double a, double b) {
...
bool image_buffer_valid(const RawCfaImage& image) {
  ...
  const auto stride = static_cast<std::size_t>(image.row_stride_pixels);
  const auto height = static_cast<std::size_t>(image.height);
  return height == 0 || stride <= std::numeric_limits<std::size_t>::max() / height
             ? image.samples.size() >= stride * height
             : false;
}
...
ShadingAnalysis analyze_shading(const RawCfaImage& image,
  ...
  analysis.field = measure_shading_field(
      image.samples.data(), image.width, image.height,
      image.row_stride_pixels, opts, analysis.signal_ceiling);
  analysis.chromatic = chromatic_response(
      analysis.field, image.color_at_position, image.cdesc, opts);
  return analysis;
}
