...
void write_model_comparison(
  ...
  w.key("noise_floor_px");
  w.value(comparison->noise_floor_px);
  w.key("noise_floor_usable");
  w.value(comparison->noise_floor_usable);
  w.key("noise_floor_source");
  w.value(comparison->noise_floor_source);
  w.key("best_overall_model");
  w.value(comparison->best_overall_model);
  w.key("parsimony_winner_model");
  w.value(comparison->parsimony_winner_model);
  w.key("conclusive");
  w.value(comparison->conclusive);
  w.key("diagnostic_conclusion");
  w.value(comparison->diagnostic_conclusion);
...
void write_independent_center_check(
  ...
  w.key("repeatability_valid_count");
