...
[relative OECF fit](OECF_FIT.md) remains explicitly outside ISO 14524
conformance.
...
This is a candidate response ladder only. The scene is non-uniform, so the
separate relative OECF report uses it only for a bounded sensor-linearity fit;
chart/patch selection and reference handling are still required for a standard
metric.
...
- This is not ISO 14524 conformance; the non-ISO
  [relative OECF fit](OECF_FIT.md) is reported separately.
- Full-frame spatial standard deviation is not treated as temporal noise.
  PTC, DSNU/PRNU, electron read noise, dark current, and dynamic range require
