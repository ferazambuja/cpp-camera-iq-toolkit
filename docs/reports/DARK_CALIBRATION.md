...
| Outlier frames | 1 |
| Set-level consistency flag | false |
...
dominated by one outlier, so the set-level consistency flag is false by design.
...
- `test_dark_calibration` covers missing reports, signed residual averaging,
  measured raw dark reconstruction, outlier counts, in-tolerance aggregates, and
  JSON fields.
