...
|---|---|---|---|---|
| RAW file metadata and admission | Covered | [RAW stats](RAW_STATS.md), [spectral report](SPECTRAL_SENSITIVITY.md) | CFA/black verification, exposure metadata checks, and private-data admission. | Maker black metadata is authoritative for measurement only after pixel decoding where required. |
...
| Spectral sensitivity extraction | Covered deeply | [Spectral report](SPECTRAL_SENSITIVITY.md) | RAW-derived recovery from monochromator sweeps, legacy-fidelity comparison, and five-camera SSF analysis. | Legacy CSVs are fidelity checks, not correctness oracles. |
| Spectral physical closure | Covered | [Spectral report](SPECTRAL_SENSITIVITY.md) | SG-140 and CC-24 physical closure for Canon/Nikon/Sony 2016 cameras using measured illuminant and reflectance. | Phase One IQ3 has SSF but no same-session broadband closure target. |
...
| SFR / MTF center ROI | Covered | [SFR result](SFR_MTF.md) | Green-linear slanted-edge MTF50P, sinc correction, D810 aperture trend, Imatest advisory comparison. | Not luma/gamma Imatest parity, lp/mm, or rendered Y-channel equivalence. |
| SFR / MTF field map | Covered | [SFR result](SFR_MTF.md) | 23-region field maps for D810 and D800, per-region advisory comparisons, field/corner gates, and the D800 negative trend finding. | Still green-linear CFA SFR; no full sagittal/tangential lens model. |
