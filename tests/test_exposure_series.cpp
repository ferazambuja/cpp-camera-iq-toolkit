...
void TESTS() {
  ...
  check(series.size() == 2, "two series above threshold");

  if (series.size() == 2) {
    // "Images/CCSG" < "Images/Dark Frame" lexically, so CCSG sorts first.
    const auto& dark = series[1];
    check(dark.directory == "Images/Dark Frame", "sorted: dark frame second");
  ...
  check(all.size() == 3, "min_distinct=2 admits the f5.6 pair");
}
