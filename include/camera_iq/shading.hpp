...
//
// Planes are indexed by 2x2 mosaic position (0,0)(0,1)(1,0)(1,1), matching
// RawCfaImage::color_at_position and cfa_stats. Each plane is measured
// independently; the planes are never summed.
//
...
// Derives the chromatic maps from a measured field. `color_at_position` holds
// LibRaw COLOR() indices for mosaic positions (0,0)(0,1)(1,0)(1,1) and `cdesc`
// maps those indices to channel letters, matching demosaic_bilinear(). Requires
// exactly one R, one B and two G positions; anything else returns `valid=false`.
ShadingChromatic chromatic_response(const ShadingField& field,
                                    const std::array<int, 4>& color_at_position,
                                    std::string_view cdesc,
                                    const ShadingOptions& opts = {});

// Production entry point. It validates the RawCfaImage contract and derives
// each signal-referred ceiling as white_level - black_per_channel[p]
// internally, preventing a caller from applying black twice or gating signed
// residuals against raw white.
struct ShadingAnalysis {
  std::array<double, 4> signal_ceiling{0, 0, 0, 0};
  ShadingField field;
  ShadingChromatic chromatic;
};

ShadingAnalysis analyze_shading(const RawCfaImage& image,
                                const ShadingOptions& opts = {});

// Pedestal verification against a dark frame. Verification only: the dark
// bounds the result and is never subtracted from, added to, or otherwise
...
// G unequally. That is why the pairing result remains explicit evidence.
//
// `exposure_metadata_matches` records whether the dark's aperture, shutter and
// ISO agree with the shading frame. A filename claiming a matching exposure is
// not provenance; a mismatch downgrades the result rather than being ignored.
...
// result computed without a dark frame carries `pedestal_unverified: true`.
...
// Serializes a primary/repeat pair and the corner-delta check as one JSON
// document. This is the output contract for the CLI's --compare mode.
...
// Long-form aggregate CSV: one row per measured value, each carrying the bin it
// came from, its channel, its units, and the acceptance state of the frame.
// Long form rather than wide because a wide table forces a reader to know the
// grid shape before a column means anything, and the grid is an option.
//
// A rejected frame still produces a table — of diagnostics only. Response and
// chromatic values are absent rather than blank, so no downstream consumer can
// read a rejected frame as a measurement with holes in it.
